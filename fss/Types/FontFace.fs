    ...
    module FontFaceClasses =
        ...
        type Src(property) =
            ...
            member this.truetype(url: string) =
                (property, Truetype url) |> FontFaceRule

            member this.opentype(url: string) =
                (property, Opentype url) |> FontFaceRule

            member this.empbeddedOpentype(url: string) =
                (property, EmpbeddedOpentype url) |> FontFaceRule

            member this.woff(url: string) = (property, Woff url) |> FontFaceRule
            member this.woff2(url: string) = (property, Woff2 url) |> FontFaceRule
            member this.svg(url: string) = (property, Svg url) |> FontFaceRule
        ...
        type FontDisplay(property) =
            ...
            member this.block = (property, Block) |> FontFaceRule
            member this.swap = (property, Swap) |> FontFaceRule
            member this.fallback = (property, Fallback) |> FontFaceRule
            member this.optional = (property, Optional) |> FontFaceRule
        // https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/font-style
        type FontStyle(property) =
            member this.normal = (property, Style.Normal) |> FontFaceRule
            member this.italic = (property, Italic) |> FontFaceRule
            member this.oblique = (property, Oblique) |> FontFaceRule

            member this.obliqueAngle(angle: Angle) =
                (property, ObliqueAngle angle) |> FontFaceRule

            member this.obliqueAngleRange(angle: Angle, range: Angle) =
                (property, ObliqueAngleRange(angle, range))
                |> FontFaceRule
        ...
        type FontStretch(property) =
            ...
            member this.ultraCondensed =
                (property, UltraCondensed) |> FontFaceRule

            member this.extraCondensed =
                (property, ExtraCondensed) |> FontFaceRule

            member this.condensed = (property, Condensed) |> FontFaceRule

            member this.semiCondensed =
                (property, SemiCondensed) |> FontFaceRule

            member this.normal = (property, Normal) |> FontFaceRule
            member this.semiExpanded = (property, SemiExpanded) |> FontFaceRule
            member this.expanded = (property, Expanded) |> FontFaceRule

            member this.extraExpanded =
                (property, ExtraExpanded) |> FontFaceRule

            member this.ultraExpanded =
                (property, UltraExpanded) |> FontFaceRule
